...
function fixtureJob(
  ...
  return {
    job,
    async load() {
      const path = `${import.meta.dir}/fixtures/${fixtureName}.md`;
...
describe("remote-europe-eligible filter (integration)", () => {
  ...
  test("yubo-hybrid → FAIL", async () => {
    const { load } = fixtureJob("yubo-hybrid", {
      company: "yubo",
  ...
  test("apify-onsite-default → FAIL", async () => {
    const { load } = fixtureJob("apify-onsite-default", {
      company: "apify",
  ...
  test("protex-no-location → FAIL", async () => {
    const { load } = fixtureJob("protex-no-location", {
      company: "Protex AI",
  ...
  test("0x-remote-crypto → PASS", async () => {
    const { load } = fixtureJob("0x-remote-crypto", {
      company: "0x",
  ...
  test("fuel-labs-remote-eu → PASS", async () => {
    const { load } = fixtureJob("fuel-labs-remote-eu", {
      company: "Fuel Labs",
  ...
  test("relay-protocol-crypto → PASS", async () => {
    const { load } = fixtureJob("relay-protocol-crypto", {
