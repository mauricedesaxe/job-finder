...
## 2. API Cost Tracking (P1)

Track Anthropic API token usage per run to catch prompt bloat or unexpected volume spikes.

### Tasks
- [ ] Capture `usage.input_tokens` and `usage.output_tokens` from every Anthropic API response (evaluate, enrich, dedup)
- [ ] Aggregate totals per run: total input tokens, total output tokens, breakdown by stage (evaluation, enrichment, dedup)
- [ ] Log estimated cost in run summary (Haiku pricing: $0.80/MTok input, $4/MTok output)
- [ ] Include token usage and cost estimate in Slack run report
