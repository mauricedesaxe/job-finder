...
- [ ] **URL canonicalization missed in dedup** (`src/pipeline/processUrl.ts`, `src/services/jobLedger.ts`) — `ledger.findByRawUrl` does not normalize host variants. Same Greenhouse listing slipped in twice via `boards.greenhouse.io/<co>/jobs/<id>` and `job-boards.greenhouse.io/<co>/jobs/<id>`. Normalize the canonical host, trailing slash, and query parameters before lookup and recording. Recurred in the 2026-05-21 review: Logos "Decentralised Messaging Engineer - Rust" (greenhouse id `7896004`) appeared twice under those two hosts.
- [ ] **Fuzzy dedup misses duplicates within a single run** (`src/pipeline/processUrl.ts`, `src/services/jobLedger.ts`) — observed in production: Harvey posted two listings with identical body (different IDs `51fb953a` / `f47e1925`) and C-Serv posted two near-identical "Senior Machine Learning Engineer" / "AI Senior Machine Learning Engineer" listings (slugs `0894153033` / `C3FBCB7264`). Concurrent same-company jobs can both read the ledger before either terminal result records its title. Add an in-run title reservation before fuzzy dedup, or run a post-record dedup pass. More examples from the 2026-05-21 review: Dev.Pro posted two near-identical Team-Lead listings (`OP02126-00` / `OP02126-01`), and Logos re-posted the same role across two Greenhouse organization boards.
